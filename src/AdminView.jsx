          ...
          overrides={
            {
              ...
              edit: { onClick: () => editAccount(), style: { cursor: "pointer" } },
...
export function Journals(props) {
  ...
  const addAccount = async (account, referenceNumber, entryDate, debit, credit) => {
    ...
    await addDoc(accountsColRef, {
      AccountName: account, ReferenceNumber: referenceNumber, EntryDate: entryDate,
  ...
  const DefaultView = (props) => {
    if (defaultView) {
      <JournalEntryForm style={{ position: 'relative', left: '30em' }}
        overrides={{
          'SelectAccount': { onChange: (event) => { setaccount(event.target.value) } },
...
export function NewAcct(props) {
  ...
  const DefaultView = (props) => {
    if (defaultView) {
      <AdminViewNewAcct style={{ position: 'relative', left: '30em' }}
        overrides={{
          'TextField34533251': { onChange: (event) => { setAccountName(event.target.value) } },
  ...
  */
  ...
  const addAccountToDB = async () => {
    await addDoc(accountsColRef, {
      acountName: accountName, accountNumber: accountNumber, accountDescription: accountDescription,
  ...
  return (
    <>
      <div style={{ position: 'absolute', alignContent: 'right' }}>
        ...
        <AdminViewNewAcct style={{ position: 'relative', left: '30em' }}
          overrides={{
            'TextField34533251': { onChange: onSubmitAccountName },
